...
function DashBoardComponent(){
  ...
  const [listPersonas, setPersonas] = useState([]); 

  const addPersona = () => {
    Axios.post("http://localhost:3301/create", {
      nombre: nombre
    }).then(() => {
      getPersonas();
  ...
  const getPersonas = () => {
    Axios.get("http://localhost:3301/personas").then((response) => {
      ...
      <div class="space-y-6 md:space-y-10 mt-10">
        ...
        <div id="profile" class="space-y-3">
          ...
          <div>
            <h2 class="font-medium text-xs md:text-sm text-center text-teal-500">
              {
                listPersonas.map((val, key) => {                  
                  return <div key={key}>{val.nombres}</div>
                })
              }
            </h2>
            <p class="text-xs text-gray-500 text-center">Developer</p>
          </div>
        </div>
        <div class="flex border-2 border-gray-200 rounded-md focus-within:ring-2 ring-teal-500">
          <input 
            ...
            placeholder="Search" />
          <button className="rounded-tr-md rounded-br-md px-2 py-3 hidden md:block" onClick={addPersona}>
